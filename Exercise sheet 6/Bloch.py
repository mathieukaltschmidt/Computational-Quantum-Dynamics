# modified from of qutip.Bloch (http://qutip.org/)
# QuTiP: Quantum Toolbox in Python
# Copyright (c) 2011 and later.
# Paul D. Nation & Robert J. Johanssonsson.

__all__ = ['Bloch', 'BlochSphere', 'BlochSpherePlot']
...
# from holoviews.plotting.mpl.element import DrawPlot
# from holoviews.plotting.mpl.chart3d import Plot3D
...
class Bloch():
    ...
    def plot_vectors(self):
        ...
        for k in range(len(self.vectors)):
            ...
            if self.vectors[0] == [1, 0, 0] and k == 0:
                # simple line style
    ...
    def plot_points(self):
        ...
        for k in range(len(self.points)):
            ...
            if any(abs(dist - dist[0]) / dist[0] > 1e-12):
                ...
                zipped = list(zip(dist, list(range(num))))
                zipped.sort()  # sort rates from lowest to highest
                dist, indperm = list(zip(*zipped))
...
"""
----------------------------------------------------------------------------
                        Bloch Sphere HoloViews
----------------------------------------------------------------------------
"""


# class BlochSphere(Element3D):
#     kdims = param.List(default=[Dimension('x'), Dimension('y'), Dimension('z')])

#     group = param.String('BlochSphere')
...
# class BlochSpherePlot(Plot3D, DrawPlot):
...
#         sphere=Bloch(axes=axis, fig=self.handles['fig'])
#         for k, v in list(self.style[self.cyclic_index].items()):
#             setattr(sphere, k, v)
